...
__all__ = ['download', 'download_mean_latent', 'download_model', 'download_cherrypicked', 'DraggableBlobMap',
           'viz_score_fn', 'norm_img', 'for_canvas', 'draw_labels', 'draw_labels_360', 'clone_layout']
...
def draw_labels(img, layout, T, colors, layout_i=0):
    ...
    blob = {k: layout[k][layout_i][mask].mul(511).tolist() for k in ('xs', 'ys')}
...
def draw_labels_360___(img, layout, T, colors, layout_i=0):
    eps = 0.000001
    PI = math.pi
    ...
    mask = layout['sizes'][layout_i, 1:] > T
    idmap = torch.arange(len(mask))[mask]
    blob = {k: layout[k][layout_i][mask] for k in ('xs', 'ys')}

    alpha = blob['xs'].mul(2 * PI).mul(-1)
    beta = blob['ys'].mul(PI).add(-PI / 2).mul(-1)
    print([1, alpha])
    print([2, beta])
    #print(alpha.shape, beta.shape)

    theta_pano = torch.zeros_like(alpha)
    phi_pano = torch.zeros_like(beta)
    ...
    print([11, torch.min(theta_pano_temp), torch.max(theta_pano_temp)])
    print([22, torch.min(phi_pano_temp), torch.max(phi_pano_temp)])

    # z rotate inverse
    theta_pano = theta_pano_temp + alpha
    phi_pano = phi_pano_temp
    print([1, torch.min(theta_pano), torch.max(theta_pano)])
    print([2, torch.min(phi_pano), torch.max(phi_pano)])

    #print([3, theta_pano_temp])
    #print([4, phi_pano_temp])

    theta_pano = (theta_pano / (2*PI) + 0.5) * 511
    phi_pano = (phi_pano / (PI)) * 511

    #theta_pano = alpha.mul(511)
    #phi_pano = torch.fmod(torch.add(beta, 1000).mul(511), 511)
    #print([3, theta_pano])
    #print([4, phi_pano])


    for i, (x, y) in enumerate(zip(theta_pano, phi_pano)):
        I = idmap[i]
        _, h = draw.textsize(str(I), font=font)
        w = h
        color = tuple(colors[I + 1].mul(255).round().int().tolist())
...
def draw_labels_360(img, layout, T, colors, layout_i=0):
    eps = 0.000001
    PI = math.pi
    score_size = 512
    rect_scaler = 1
    ...
    xs_sample = blob['xs']
    alpha = blob['xs']  # [bs, num_blobs, 1]
    alpha = torch.unsqueeze(alpha.mul(2 * PI), -1)  # [bs, num_blobs, 1, 1]
    #print(['alpha_outside', alpha.shape, torch.max(alpha), torch.min(alpha), torch.mean(alpha)])

    beta = blob['ys']  # [bs, num_blobs, 1]
    beta = torch.unsqueeze(beta.mul(PI).add(-PI / 2), -1)  # [bs, num_blobs, 1, 1]
    #print(['beta_outside', beta.shape, torch.max(beta), torch.min(beta), torch.mean(beta)])

    grid_coords = torch.stack(
        (torch.arange(start=-PI + eps, end=PI - eps, step=(2 * PI - 2 * eps) / (score_size * rect_scaler)).repeat(
            score_size),
         torch.arange(start=0 + eps, end=PI - eps, step=(PI - 2 * eps) / score_size).repeat_interleave(
             score_size * rect_scaler))
    ).to(xs_sample.device)


    feature_coords = torch.stack((torch.zeros_like(blob['xs']), torch.zeros_like(blob['ys'])),
                                 -1).to(xs_sample.device)  # [bs, num_blobs, 2]

    delta = grid_coords[None, None] - feature_coords[..., None]  # [bs, num_blobs, 2(x,y), size*size]
    #print(['delta_outside', delta.shape, torch.max(delta), torch.min(delta), torch.mean(delta)])
    ...
    #print(['polar_r_outside', polar_r.shape, torch.max(polar_r), torch.min(polar_r), torch.mean(polar_r)])

    sq_mahalanobis = polar_r
    sq_mahalanobis = torch.squeeze(einops.rearrange(sq_mahalanobis, 'n m (s1 s2) -> n s1 s2 m', s1=score_size)) #[1, 512, 512, 16])

    #print(sq_mahalanobis.shape)
    xss = []
    yss = []
    for i in range(sq_mahalanobis.shape[-1]):
        blob_i_distance = sq_mahalanobis[:, :, i]
        blob_i_index = (blob_i_distance == torch.min(blob_i_distance)).nonzero()[0]
        yss.append(blob_i_index[0])
        xss.append(blob_i_index[1])

        #print(blob_i_index.shape, blob_i_index)
