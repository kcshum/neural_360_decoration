...
def splat_features_from_scores(scores: Tensor, features: Tensor, size: Optional[int],
    ...
    if size and not (scores.shape[2] == size):
        ...
        scaler = int(scores.shape[3] / scores.shape[2])
        scores = F.interpolate(scores, size=[size, size * scaler], mode='bilinear', align_corners=False)
...
def pyramid_resize(img, cutoff):
    ...
    """
    out = [img]
    while img.shape[-2] > cutoff:
        img = F.interpolate(img, size=[img.shape[-2] // 2, img.shape[-1] // 2], mode='bilinear', align_corners=False)
        out.append(img)
    return {i.size(-2): i for i in out}
