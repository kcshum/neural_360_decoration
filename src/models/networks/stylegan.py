...
class Blur(nn.Module):
    def __init__(self, kernel, pad, use_horizontal_circular_padding=False, upsample_factor=1):
        super().__init__()
        kernel = make_kernel(kernel)
        ...
        if self.use_horizontal_circular_padding:
            print('Generator Blur using circular pad !')

        self.pad = pad

        assert len(pad) == 2
        #print([kernel, upsample_factor, self.pad])

    def forward(self, input):
        if self.use_horizontal_circular_padding and self.pad[0] > 0 and self.pad[1] > 0:
            hor_pad = (self.pad[0], self.pad[1], 0, 0)
            out = F.pad(input=input, pad=hor_pad, mode='circular')

            ver_pad = (0, 0, self.pad[0], self.pad[1])
            out = upfirdn2d(out, self.kernel, pad=ver_pad)
        else:
...
class EqualConv1d(nn.Module):
    def __init__(
            self, in_channel, out_channel, kernel_size, stride=1, padding=0, bias=True, lr_mul=1.
    ):
        super().__init__()

        self.weight = nn.Parameter(
            torch.randn(out_channel, in_channel, kernel_size).div_(lr_mul)
        )
...
def StyleMLP(n_layers, dim, lr_mul=1, first_dim=None, last_dim=None, pixel_norm=True, last_relu=True):
    ...
    for i in range(n_layers):
        _layers.append(EqualLinear(dim if i else first_dim, dim if i + 1 < n_layers else last_dim,
                                   activation="fused_lrelu" if (i + 1 < n_layers or last_relu) else False,
                                   lr_mul=lr_mul))
...
class StyleGANGenerator(nn.Module):
    def __init__(
            ...
            lr_mlp=0.01,
            c_out=3,
            latent_to_img_space='w',
            **kwargs
    ):
        super().__init__()
        # kwargs are additional properties/metadata (e.g. latent space dim), stash them as attributes
...
class StyleGANDiscriminator(nn.Module):
    def __init__(self, size, rect_scaler=1, channel_multiplier=2, blur_kernel=[1, 3, 3, 1], discriminate_stddev=True, in_channels=3,
        ...
        self.final_linear = nn.Sequential(
            EqualLinear(channels[4] * 4 * 4 * rect_scaler, channels[4], activation="fused_lrelu"),
