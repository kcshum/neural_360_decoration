...
class LayoutStyleGANGenerator(nn.Module):
    ...
    use_horizontal_circular_padding: bool = False
    ...
    def __post_init__(self):
        ...
        self.c_origin = self.c_origin + self.c_indicator
        self.channels = {
            4: self.c_model * 8 + self.c_origin,
            8: self.c_model * 8 + self.c_origin,
            16: self.c_model * 8 + self.c_origin,
            32: self.c_model * 8 + self.c_origin,
            64: self.c_model * 4 * self.channel_multiplier + self.c_origin,
            128: self.c_model * 2 * self.channel_multiplier + self.c_origin,
            256: self.c_model * self.channel_multiplier + self.c_origin,
            512: self.c_model // 2 * self.channel_multiplier + self.c_origin,
            1024: self.c_model // 4 * self.channel_multiplier + self.c_origin,
        }
        ...
        self.conv1 = SpatialStyledConv(
            self.override_c_in or self.channels[self.size_in], self.out_channels[self.size_in], 3, self.style_dim, blur_kernel=blur_kernel,
            use_horizontal_circular_padding=self.use_horizontal_circular_padding
        )
        ...
        for i in range(self.log_size_in + 1, self.log_size + 1):
            out_channel = self.out_channels[2 ** i]
            ...
            self.convs.append(
                SpatialStyledConv(
                    out_channel + self.c_origin, out_channel, 3, self.style_dim, blur_kernel=blur_kernel,
                    use_horizontal_circular_padding=self.use_horizontal_circular_padding
                )
            )

            self.to_rgbs.append(SpatialToRGB(out_channel + self.c_origin, self.style_dim, c_out=self.c_out,
                                             use_horizontal_circular_padding=self.use_horizontal_circular_padding))

            in_channel = self.channels[2 ** i]
...
class SpatialToRGB(nn.Module):
    def __init__(self, in_channel, style_dim, use_horizontal_circular_padding, upsample=True, blur_kernel=[1, 3, 3, 1], c_out=3):
        super().__init__()

        if upsample:
            self.upsample = Upsample(blur_kernel)

        self.conv = SpatialModulatedConv2d(in_channel, c_out, 1, style_dim, demodulate=False, use_horizontal_circular_padding=use_horizontal_circular_padding)
...
class SpatialModulatedConv2d(nn.Module):
    def __init__(
        ...
        if self.use_horizontal_circular_padding:
            print('Generator Conv using circular pad !')
        ...
        if upsample:
            ...
            self.blur = Blur(blur_kernel, pad=(pad0, pad1), use_horizontal_circular_padding=self.use_horizontal_circular_padding, upsample_factor=factor)
        ...
        if downsample:
            ...
            self.blur = Blur(blur_kernel, pad=(pad0, pad1), use_horizontal_circular_padding=self.use_horizontal_circular_padding)
        ...
        self.modulation = EqualLinear(style_dim, in_channel - 3, bias_init=1)
    ...
    def forward(self, input, style):
        ...
        if spatial_style or not self.fused:
            if spatial_style:
                ...
                style = splat_features_from_scores(layout['scores_pyramid'][input.size(-2)], style, input.size(-2),
                                                   channels_last=False)
                if self.demodulate:
            ...
            input = input * style
            input = torch.cat((input, layout['input_pyramid'][input.size(-2)]), 1)
            ...
            if self.upsample:
                ...
                if self.use_horizontal_circular_padding and self.padding > 0:
                    ...
                    out = F.pad(input=input, pad=hor_pad, mode='circular')

                    ver_pad = (self.padding, 0)
                    out = conv2d_gradfix.conv2d(out, weight, padding=ver_pad)
                else:
